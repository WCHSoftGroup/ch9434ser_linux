/*
 * UART and GPIO application example for SPI to Quad UARTs chip ch9434.
 *
 ...
 * V1.2 - changed RS485 operation method
 */
...
static const struct option lopts[] = {
	{ "device", required_argument, 0, 'D' },   { "speed", optional_argument, 0, 'S' },
	{ "verbose", optional_argument, 0, 'v' },  { "hardflow", required_argument, 0, 'f' },
	{ "savefile", required_argument, 0, 's' }, { NULL, 0, 0, 0 },
};

static void print_usage(const char *prog)
{
	printf("Usage: %s [-DSvfRs]\n", prog);
...
static void parse_opts(int argc, char *argv[])
{
	...
	while (1) {
		c = getopt_long(argc, argv, "D:S:vfRsh", lopts, NULL);
		if (c == -1) {
			break;
		}
		switch (c) {
...
int speed_arr[] = { B4000000, B3000000, B2500000, B2000000, B1000000, B921600, B460800, B230400,
		    B115200,  B57600,	B38400,	  B19200,   B9600,    B4800,   B2400,	B1200,
		    B600,     B300,	B200,	  B150,	    B110,     B75,     B50 };

int name_arr[] = { 4000000, 3000000, 2500000, 2000000, 1000000, 921600, 460800, 230400, 115200, 57600, 38400, 19200,
		   9600,    4800,    2400,    1200,    600,	300,	200,	150,	110,	75,    50 };
...
int libtty_rs485set(int fd, char enable)
{
	...
	return ioctl(fd, IOCTL_CMD_SRS485, &rs485conf);
}
...
{
	return ioctl(fd, IOCTL_CMD_GRS485, rs485conf);
}
