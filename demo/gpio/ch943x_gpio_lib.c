...
static int is_gpio_number_valid(CHIPTYPE chiptype, uint8_t gpionumber)
{
    if ((chiptype == CHIP_CH9434D) && (gpionumber > 3))
        return -ENXIO;
    else if ((chiptype == CHIP_CH9432D) && (gpionumber > 7))
        return -ENXIO;
    else if ((chiptype == CHIP_CH9438F) && (gpionumber > 7))
        return -ENXIO;
    else if ((chiptype == CHIP_CH9437F) && (gpionumber > 10))
        return -ENXIO;
...
int ch943x_single_gpioenable(int fd, uint8_t gpionumber, uint8_t enable)
{
    ...
    case CHIP_CH9434M:
        ...
        val2 = enable ? (val2 | BIT(gpionumber % 8)) : (val2 & ~BIT(gpionumber % 8));
    ...
    /* default high level */
    if ((chiptype == CHIP_CH9434D) || (chiptype == CHIP_CH9432D) || (chiptype == CHIP_CH9438F) ||
        (chiptype == CHIP_CH9437F)) {
